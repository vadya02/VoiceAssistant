...
const VoiceRecorderWAV = ({ getHistory }) => {
	...
	const uploadAudio = (audioBlob) => {
		...
		try {
			...
			axios
				...
				.then((response) => {
					// response.data содержит распарсированный JSON
					console.log(response.data);
					console.log(response.data.url);
					console.log(response.data.text);
					// if (response.data.message_error) {
					// 	store.setResponse(response.data.message_error);
					// } else {
	...
	return (
		<div className="p-2">
			<Row className="d-flex justify-content-center align-items-center p-2">
				<VoiceVisualizer ref={audioRef} mainBarColor="black" controls={recorderControls} />
